...
function autoDownLoadAllImages() {
    // 使用谷歌浏览器打开页面,打开调试工具，控制台面板粘贴以下代码，完成自动下载这个页面已经加载的图片（包括背景图）
    var a = [];
    ...
    [].forEach.call(imgs, function (item, index) {
        try {
            var name = /(.+)\.(gif|jpg|png)/gi.exec(item.src);
            var finish = name[1];
            var end = name[2];
            a.push('<a href="' + item.src + '" download="' + finish + '.' + end + '" class="xiazai"></a>')
        } catch (error) {

        }
    });
    [].forEach.call(document.querySelectorAll(' *'), function (item, index) {
        var bg = getComputedStyle(item)['backgroundImage'];
        try {
            var name = /(.+)\.(gif|jpg|png)/gi.exec(bg);
            var finish = name[1];
            var end = name[2];
            if (bg !== 'none') a.push('<a href="' + item.src + '" download="' + finish + '.' + end + '" class="xiazai"></a>');
        } catch (error) {

        }
    });
    var d = a.unique();
    document.body.innerHTML += d.join('');
    var c = document.querySelectorAll('.xiazai');
    var num = 0;
    var b = c.length;
...
function transDom(str) {
    var tem = document.createElement('div');
    tem.innerHTML = str;

    return tem.childNodes.length === 1 ? tem.childNodes[0] : tem.childNodes;
}
 ...
 */
function getClassDOM(clsname,parent) {
    var allObjs;
    if(parent){
        allObjs=parent.getElementsByTagName('*');
    }else{
    ...
    for (var i = 0; i < allObjs.length; i++) {
        ...
        if (reg.test(allObjs[i].className)) {

            classObjs.push(allObjs[i]);
        }
    }
    return classObjs.length === 1 ? classObjs[0] : classObjs;
}
